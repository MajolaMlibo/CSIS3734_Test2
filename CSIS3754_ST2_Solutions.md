# CSIS3754 – Semester Test 2 Solutions
...
>   - **Question 1**: Answer on the answer sheet provided. *(Closed Book)*
>   - **Question 2**: Create and submit a Jupyter Notebook. *(Open Book)*
> - Submit the notebook to: `CSIS3754 → Assessments → Semester Test 2 → Question 2`
...
### 1.1 Learning Curves: SVM vs KNN

> **Context:** Two learning curves are provided — the left shows an SVM classifier and the right shows a KNN classifier.

---

#### 1.1.1 Which model would most probably provide better prediction results? *(1 mark)*

> **✅ Answer:**
>
> The **Support Vector Machine (SVM)** classifier would most probably provide better prediction results.

---

#### 1.1.2 Motivation *(3 marks)*

> **✅ Answer:**
>
> The SVM model is preferred because its learning curve shows that the **training score and cross-validation score have converged** (the gap between them is small). This indicates that the model generalises well — it performs similarly on both seen (training) and unseen (cross-validation) data, which is the hallmark of a well-fitted model.
>
> The KNN classifier's learning curve shows a **large gap between the training score (~0.75) and the cross-validation score (~0.60)**. This gap indicates **overfitting** — the model memorises the training data but fails to generalise to new data. Therefore, the SVM is the better-performing model.

---

### 1.2 k-Means Clustering Algorithm Steps *(5 marks)*

> **✅ Answer:**
>
> The k-Means algorithm groups data into clusters through the following steps:
>
> 1. **Choose k:** Select the number of clusters *k* to form.
> 2. **Initialise centroids:** Randomly place *k* centroids within the data space (one per intended cluster).
> 3. **Assign data points:** Calculate the distance from each data point to every centroid. Assign each data point to the cluster whose centroid is closest (typically using Euclidean distance).
> 4. **Recalculate centroids:** Once all points are assigned, compute the new centroid of each cluster by calculating the mean position of all data points in that cluster.
> 5. **Repeat:** Repeat steps 3 and 4 iteratively until the centroids no longer move significantly (convergence), meaning cluster assignments do not change between iterations.

---

### 1.3 Decision Tree – Gini Impurity
...
> **Total records: 7**
> **HA = Yes: 3** (rows 1, 5, 7)
> **HA = No: 4** (rows 2, 3, 4, 6)

---

#### 1.3.1 Determine the root feature using Gini Impurity *(11 marks)*

> **✅ Full Calculation:**
>
> **Formula:** `Gini(node) = 1 – Σ(pᵢ)²`
>
> ---
>
> **Step 1: Gini Impurity of the full dataset (parent node)**
>
> - P(HA=Yes) = 3/7
> - P(HA=No) = 4/7
>
> `Gini(parent) = 1 – (3/7)² – (4/7)²`
> `= 1 – 9/49 – 16/49`
> `= 1 – 25/49`
> `= 24/49 ≈ 0.4898`
>
> ---
>
> **Step 2: Split on HBP (High Blood Pressure)**
>
> | HBP = Yes | HBP = No |
> |-----------|----------|
> | Rows: 1, 3, 5, 6, 7 → 5 records | Rows: 2, 4 → 2 records |
> | HA=Yes: 3 (rows 1,5,7) | HA=Yes: 0 |
> | HA=No: 2 (rows 3,6) | HA=No: 2 |
>
> `Gini(HBP=Yes) = 1 – (3/5)² – (2/5)² = 1 – 9/25 – 4/25 = 12/25 = 0.48`
>
> `Gini(HBP=No) = 1 – (0/2)² – (2/2)² = 1 – 0 – 1 = 0`
>
> **Weighted Gini for HBP:**
> `Gini(HBP) = (5/7) × 0.48 + (2/7) × 0`
> `= 0.3429 + 0`
> `= 0.3429`
>
> **Gini Gain (HBP) = 0.4898 – 0.3429 = 0.1469**
>
> ---
>
> **Step 3: Split on Age < 48**
>
> | Age < 48 = Yes | Age < 48 = No |
> |----------------|---------------|
> | Rows: 1,2,3,4,6 → 5 records | Rows: 5, 7 → 2 records |
> | HA=Yes: 1 (row 1) | HA=Yes: 2 (rows 5,7) |
> | HA=No: 4 (rows 2,3,4,6) | HA=No: 0 |
>
> `Gini(Age=Yes) = 1 – (1/5)² – (4/5)² = 1 – 1/25 – 16/25 = 8/25 = 0.32`
>
> `Gini(Age=No) = 1 – (2/2)² – (0/2)² = 1 – 1 – 0 = 0`
>
> **Weighted Gini for Age < 48:**
> `Gini(Age) = (5/7) × 0.32 + (2/7) × 0`
> `= 0.2286 + 0`
> `= 0.2286`
>
> **Gini Gain (Age) = 0.4898 – 0.2286 = 0.2612**
>
> ---
>
> **Step 4: Compare Gini Gains**
>
> | Feature    | Gini Gain |
> |------------|-----------|
> | HBP        | 0.1469    |
> | Age < 48   | **0.2612** |
>
> **✅ Root Feature = Age < 48**, because it produces the **highest Gini Gain** (i.e., the lowest weighted Gini Impurity after the split), meaning it provides the most information to separate the classes.
...
> **Notebook Name:** `CSIS3754_YourStudentNumber_ST2-2_2025`
...
**Date:** 22 May 2024 | **Time:** 180 min | **Total Marks:** 90
...
>   - **Question 1**: Answer on the answer sheet provided. *(Closed Book)*
>   - **Question 2**: Create and submit a Jupyter Notebook. *(Open Book)*
>   - **Question 3**: Create and submit a Jupyter Notebook. *(Open Book)*
> - Submit notebooks to:
>   - `CSIS3754 → Assessments → Semester Test 2 → Question 2`
>   - `CSIS3754 → Assessments → Semester Test 2 → Question 3`
...
### 1.1 Learning Curves: SVM vs KNN *(Same as 2025)*

#### 1.1.1 Which model provides better predictions? *(1 mark)*

> **✅ Answer:** The **Support Vector Machine (SVM)** classifier.

#### 1.1.2 Motivation *(3 marks)*

> **✅ Answer:**
>
> The SVM's learning curve shows that the **training score and cross-validation score have converged** with a small gap between them, indicating the model generalises well to unseen data with minimal overfitting.
>
> The KNN learning curve shows a **large gap between training accuracy (~0.75) and cross-validation accuracy (~0.60)**, which is a clear sign of **overfitting**. The KNN model memorises the training data but does not perform well on new, unseen data.

---

### 1.2 Agglomerative Hierarchical Clustering – Dendrogram *(4 marks)*

> **Scatter plot points** (approximate positions):
> - **Group 1 (top-left):** A, B, C (close together)
> - **Group 2 (middle):** D, E, F, G (close together)
> - **Group 3 (right):** H, I (somewhat isolated)

> **✅ Single-Linkage (Minimum Distance) Dendrogram Construction:**
>
> **Step 1 – Find the two closest points:**
> B and C are closest → merge into cluster **{B,C}**
>
> **Step 2 – Next closest:**
> D and E are very close → merge into **{D,E}**
>
> **Step 3 – Next:**
> F and G are close → merge into **{F,G}**
>
> **Step 4 – Next:**
> {B,C} is close to A → merge into **{A,B,C}**
>
> **Step 5 – Next:**
> {D,E} is close to {F,G} → merge into **{D,E,F,G}**
>
> **Step 6 – Next:**
> H and I merge → **{H,I}**
>
> **Step 7 – Merge middle cluster with H,I:**
> {D,E,F,G} merges with {H,I} → **{D,E,F,G,H,I}**
>
> **Step 8 – Final merge:**
> {A,B,C} merges with {D,E,F,G,H,I} → **{A,B,C,D,E,F,G,H,I}**
>
> ```
> Distance
>    |                                     ┌─────────┐
...
> ```
>
> *Note: In an exam, draw vertical lines at each merge level with correct spacing. The dendrogram above represents the logical merge sequence using single-linkage.*

---

### 1.3 Decision Tree – Gini Impurity *(12 marks)*

> **Same dataset as 2025 paper:**
>
> | HBP | Age < 48 | HA  |
> |-----|----------|-----|
> | Yes | Yes      | Yes |
...
#### 1.3.1 Determine root feature using Gini Impurity *(12 marks)*

> **✅ Full Calculation** *(identical dataset to 2025 — same answer applies)*
>
> **Parent Gini:**
> - P(HA=Yes) = 3/7, P(HA=No) = 4/7
> - `Gini(parent) = 1 – (3/7)² – (4/7)² = 1 – 9/49 – 16/49 = 24/49 ≈ 0.4898`
>
> ---
>
> **Split on HBP:**
>
> | Branch | Records | HA=Yes | HA=No | Gini |
> |--------|---------|--------|-------|------|
> | HBP = Yes | 5 | 3 | 2 | `1–(3/5)²–(2/5)² = 0.48` |
> | HBP = No  | 2 | 0 | 2 | `1–0–(2/2)² = 0.00` |
>
> `Weighted Gini(HBP) = (5/7)×0.48 + (2/7)×0 = 0.3429`
> `Gini Gain(HBP) = 0.4898 – 0.3429 = 0.1469`
>
> ---
>
...
> | Branch | Records | HA=Yes | HA=No | Gini |
> |--------|---------|--------|-------|------|
> | Age < 48 = Yes | 5 | 1 | 4 | `1–(1/5)²–(4/5)² = 0.32` |
> | Age < 48 = No  | 2 | 2 | 0 | `1–(2/2)²–0 = 0.00` |
>
> `Weighted Gini(Age) = (5/7)×0.32 + (2/7)×0 = 0.2286`
> `Gini Gain(Age) = 0.4898 – 0.2286 = 0.2612`
>
> ---
>
...
> | Age < 48 | **0.2612** |
>
> **✅ Root Feature = Age < 48** — it has the highest Gini Gain and therefore provides the most information for splitting the data.
